        ...
        <div class="grid grid-cols-7 gap-4 p-5">
            @for ($i=0; $i<35; $i++)
                @php
                    ...
                    $ux = strtotime($dat);
                @endphp
                <div class="ux_{{ $ux }} day h-32 border text-right p-1 border-slate-500 {{ $class }}">
                    <div class="h-4 text-sm font-bold">{{ $dat  }}</div>
                    <div class="h-28 overflow-hidden">
                        ...
                        @foreach ($ev[$dat] as $e)
                            @php
                                switch ($e->event_type){
                                    case ('ph'):
                                        $event_class="bg-fuchsia-500";
                                        break;
                                    case('usr'):
                                        $event_class="bg-blue-300";
                                        break;
                                    default:
                                        $event_class="bg-green-500";
                                        break;
                                }
                            @endphp
                            <a href="#">
                            <div class="event w-full text-xs border border-slate-700 pl-1 text-left mb-1 {{$event_class}}" >
    ...
    <div class="fixed z-10 inset-0 invisible overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" id="interestModal">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <form method="POST" action="{{ route("cal.saveEvent") }}">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    ...
                                    From: <input type="text" name="event_time_start" value="2022-11-16 15:00:00"><br>
                                    To: Event: <input type="text" name="event_time_end" value="2022-11-16 16:00:00">
